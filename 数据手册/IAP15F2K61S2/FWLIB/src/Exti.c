...
/********************* INT2中断函数 *************************/
void Ext_INT2 (void) interrupt INT2_VECTOR	//P36引脚  SIM300本地接听按键
{
	INT_CLKO &= ~(1 << 4);	//禁止中断
	if(RING_Status==1)
	{
		if((RING_Mode==0)&&(flag==0))
		{	
			if(!Set_Mode("ATA\r\n"))	
			{
				P27=0;
				SW_A=0;SW_B=0;		 /*CD4053模拟开关——SIM300输出到功放*/
				flag=1;				 //接听 
			}				   	
		}																	
		else if((RING_Mode==0)&&(flag==1))			
		{	 
			if(!Set_Mode("ATH\r\n"))	
			{
				P27=1;
